...
def get_image_from_url(url: str) -> ndarray:
    ...
    pil_image: Image = Image.open(BytesIO(response_data.content), mode="r").convert("RGB").filter(ImageFilter.GaussianBlur(1)).quantize(quantized_level).resize((16, 16))
...
if __name__ == "__main__":
    ...
    size_threshold = round(0.01 * image.shape[0] * image.shape[1])
    n_blobs, blob = blob_extract(array(image))
    table = [
        [image[i][j], table[blob[i][j] - 1][1] + 1] if blob[i][j] != 0 else [0, 0]
